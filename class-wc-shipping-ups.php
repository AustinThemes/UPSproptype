...
/*
* ================================================= START FSG RESIDENTIAL API CHECK FUNCTIONS ========================
* FireSideGames.com UPS Residential vs Commercial Check Script
* Credit to Original API call snippet Author  : Mark Song (ilovekanon@gmail.com) @ uniapple.net
*/

/* API ACCESS CREDENTIALS */
define("ACCESS_LICENSE_NUMBER","0D0A560D525CE036");
define("USER_ID","AustinThemes");
define("USER_PASSWORD","vLAPE@REqELQr7");
define("MODE","Live"); //Dev or Live ( If set to Dev, Only NY and CA results will be returned.)
...
<AddressValidationRequest xml:lang='en-US'>
  ...
  <AddressKeyFormat>
    ...
    if($this->_address2) $xml .= "<AddressLine>#2L</AddressLine>";
    $xml .= "
...
/* FUNCTION TO PASS THE CLASS THE ACTUAL ADDRESS WITH VARS AND RETURN RESULTS */
function FireSideGames_CheckUPSaddresstype($FSGXAV_address,$FSGXAV_city,$FSGXAV_state,$FSGXAV_zip){
try{
$av = new Address_validation();
$av->setReturnType('Array'); // Array or json (Default)
$address = array("address1" => $FSGXAV_address,
"city"	=> $FSGXAV_city,
"state" => $FSGXAV_state,
"zipcode" => $FSGXAV_zip,
"country"	=> "US");
...
$data = $av->getResponse();
$FSGXAV_address_type = $data[0]['description'];
if ($FSGXAV_address_type == 'Residential'){ return true; } else {return false; }
}catch(Exception $e){
return true; //return residential status as true to cover errors or unclassified addresses
}
} //CLOSE FUNCTION


/* =============================== END FSG API RESIDENTIAL CHECK FUNCTIONS============================  */
  ...
  */
  ...
  if ( ( ! $this->user_id || ! $this->password || ! $this->access_key || ! $this->shipper_number ) && $this->enabled == 'yes' ) {
	...
	private function get_rate_requests( $package_requests, $package ) {
      ...
      //FireSideGames - Query UPS Street Level API to Check if Residential Address in advance
      $FSGXAV_function_query_address = $package['destination']['address'];
      $FSGXAV_function_query_city = $package['destination']['city'];
      $FSGXAV_function_query_state = $package['destination']['state'];
      $FSGXAV_function_query_zipcode = $package['destination']['postcode'];
      
      if (FireSideGames_CheckUPSaddresstype($FSGXAV_function_query_address, $FSGXAV_function_query_city, $FSGXAV_function_query_state, $FSGXAV_function_query_zipcode)) { 
      $FSG_ResidentialFlag = "			<ResidentialAddressIndicator></ResidentialAddressIndicator>" . "\n"; 
      } else { 
      $FSG_ResidentialFlag = ""; 
      };
       ...
       /* Firesidegames - include residential tag conditionally */ 				
				$request .= $FSG_ResidentialFlag;
